...
can_transmit_state_enum can_transmit_states(uint32_t can_periph, uint8_t mailbox_number)
{
    ...
    case CAN_MAILBOX0:
        val = CAN_TSTAT(can_periph) & (CAN_TSTAT_MTF0 | CAN_TSTAT_MTFNERR0 | CAN_TSTAT_TME0 | CAN_TSTAT_MTE0);
        if((val&CAN_TSTAT_MTE0) != 0)
        {
          return CAN_TRANSMIT_ERROR;
        }
        break;
    case CAN_MAILBOX1:
        val = CAN_TSTAT(can_periph) & (CAN_TSTAT_MTF1 | CAN_TSTAT_MTFNERR1 | CAN_TSTAT_TME1 | CAN_TSTAT_MTE1);
        if((val&CAN_TSTAT_MTE1) != 0)
        {
          return CAN_TRANSMIT_ERROR;
        }
        break;
    case CAN_MAILBOX2:
        val = CAN_TSTAT(can_periph) & (CAN_TSTAT_MTF2 | CAN_TSTAT_MTFNERR2 | CAN_TSTAT_TME2 | CAN_TSTAT_MTE2);
        if((val&CAN_TSTAT_MTE2) != 0)
        {
          return CAN_TRANSMIT_ERROR;
        }
        break;
